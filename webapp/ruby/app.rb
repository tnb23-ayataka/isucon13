      ...
      livestreams = db_transaction do |tx|
        livestream_models =
          if key_tag_name != ''
            ...
            tags = tx.xquery("SELECT * FROM livestream_tags lt INNER JOIN tags t ON lt.tag_id = t.id WHERE lt.tag_id IN (#{tag_id_list.map {'?'}.join(',')}) ORDER BY lt.livestream_id desc", tag_id_list)
